...
def fill_depth_colorization(rgb_filename=None, depth_filename=None, alpha=1):

	# Open the image form working directory
	rgb_image = Image.open(rgb_filename)
	# convert image to numpy array
	imgRgb = np.asarray(rgb_image)

	# Open the image form working directory
	depth_image = Image.open(depth_filename)
	# convert image to numpy array
	imgDepthInput = np.asarray(depth_image) / 256
	...
	# Fill_Dpeth logic ends 
	output_image = Image.fromarray(output.astype('uint8'))
	output_image.save(depth_filename)
...
if __name__ == '__main__':
	...
	directory_depth = '/content/depth/'
	directory_rgb = '/content/rgb/'
	rgb_array = []
	depth_array = []

	for filename in os.listdir(directory_depth):
		if filename.endswith(".jpg") or filename.endswith(".png"):
			rgb_array.append(directory_rgb + filename)
			depth_array.append(directory_depth + filename)

	pbar_end = tqdm(total=len(depth_array))
	
	p = Pool()
	p.starmap(fill_depth_colorization, zip(rgb_array,depth_array))